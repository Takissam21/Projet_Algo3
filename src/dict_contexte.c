...
/**
 * Cette fonction renvoie le la liste des contexte du mot passé en entrée, dans le dicitonnaire d
 * REMAQUE: on part du principe que le mot à bel et bien on contexte dans d
 */
...
char* maxContexte(dictContexte d, char* m) {
	...
	while ((contexte_cour->freq_suivante)->freq_suivante != NULL) {
...
/**
 * Cette foncion renvoie le nombre de fois qu'apparaît de mot n dans le donctionnaire des contextes de m
 */
